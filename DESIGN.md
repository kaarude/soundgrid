...
- **Label** (600, 11px, 1, +1px tracking, UPPERCASE): structural metadata — bus labels, card categories, settings section headers. The tracked-uppercase label is the one typographic flourish, and it is reserved for *naming* things (buses, categories), not for decoration.
...
- **Don't** pair a 1px border with a wide (≥16px blur) drop shadow on the same element. Pick the Hairline border *or* the Ambient Lift shadow, never both as decoration.
